...
def get_url_by_kangnam(day, idx, is_kangnam):
...
def get_by_room_time(idx, is_kangnam):
    if is_kangnam:
...
class DanPyeon:
    ...
    def login(self, idx, day, user_id, pwd, is_kangnam=True):
        idx = get_by_room_time(idx, is_kangnam)
        url = get_url_by_kangnam(day, idx, is_kangnam)
    ...
    def reservation(self, phone_number, person, is_kangnam=True):
        ...
        agree_cancel = self.driver.find_element(By.XPATH,
                                                '//*[@id="order_form_wrap"]/div[1]/div[4]/div/div/div/div/div/label/span')
        agree_cancel.click()

        # id 속성이 'paymentAllCheck'인 체크박스 선택 및 클릭
        payment_all_check = self.driver.find_element(By.XPATH,
                                                     '//*[@id="order_form_wrap"]/div[2]/div[4]/div/div[1]/label/span')
