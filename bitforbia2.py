...
def start_waiting():
    ...
    reservation_data["phone1"] = entry_phone1.get()
    reservation_data["phone2"] = entry_phone2.get()
    reservation_data["phone3"] = entry_phone3.get()
...
def perform_crawling():
    ...
    try:
        ...
        try:
            # 테마별 박스 요소 찾기
            theme_boxes = driver.find_elements(By.CLASS_NAME, "box")

            for box in theme_boxes:
                # 테마 제목 찾기
                title_element = box.find_element(By.CLASS_NAME, "tit")
                title = title_element.text.strip()

                if title == combo_theme.get():
                    # 해당 테마 내의 모든 예약 가능한 시간 찾기
                    time_elements = box.find_elements(By.CLASS_NAME, "sale")

                    for time_element in time_elements:
                        # 시간 값 추출
                        time_text = time_element.text.strip()

                        if time_text == combo_time.get():
                            print(f"클릭 시도: {combo_theme.get()} - {combo_time.get()}")
                            time_element.click()

                            driver.implicitly_wait(30)
                            # 인원 선택 (Dropdown)
                            person_select = Select(driver.find_element(By.ID, "person"))
                            person_select.select_by_value(str(reservation_data["people"]))
                            print(f"인원 선택 완료: {reservation_data['people']}명")

                            # 예약자 이름 입력
                            name_input = driver.find_element(By.NAME, "name")
                            name_input.clear()
                            name_input.send_keys(combo_people.get())
                            print(f"예약자 이름 입력 완료: {combo_people.get()}")

                            # 연락처 입력
                            driver.find_element(By.NAME, "mobile1").clear()
                            driver.find_element(By.NAME, "mobile1").send_keys(entry_phone1.get())
                            driver.find_element(By.NAME, "mobile2").clear()
                            driver.find_element(By.NAME, "mobile2").send_keys(entry_phone2.get())
                            driver.find_element(By.NAME, "mobile3").clear()
                            driver.find_element(By.NAME, "mobile3").send_keys(entry_phone3.get())
                            print(f"연락처 입력 완료: {entry_phone1.get()}-{entry_phone2.get()}-{entry_phone3.get()}")

                            # 쿠폰 코드 입력 (옵션)
                            if entry_coupon.get():
                                coupon_input = driver.find_element(By.ID, "coupon")
                                coupon_input.clear()
                                coupon_input.send_keys(entry_coupon.get())
                                print(f"쿠폰 코드 입력 완료: {entry_coupon.get()}")

                                # 쿠폰 적용 버튼 클릭
                                coupon_button = driver.find_element(By.XPATH, "//span[@id='coupon_exe']/button")
                                coupon_button.click()
                                print("쿠폰 적용 버튼 클릭 완료")


                            all_check = driver.find_element(By.XPATH, '//*[@id="contents"]/div/div/div[1]/div[2]/div[2]/form/div[2]/div[1]/a')
                            all_check.click()
                            # 자동등록방지 (캡차) 입력란에 포커스 맞추기
                            captcha_input = driver.find_element(By.ID, "spam")
                            captcha_input.click()  # 클릭하여 포커스를 맞춤
                            print("⚠️ 캡차 입력란에 포커스를 맞췄습니다! 브라우저에서 직접 입력하세요.")
                            return True

            print(f"예약 가능한 {combo_theme.get()}의 {combo_time.get()} 시간 슬롯이 없습니다.")
            return False
